...
which is a g++ wrapper that automatically adds everything the engine needs to the provided options and forwards them to g++.   

-d[\<option\>], -r[\<option\>], -l[\<option\>] and -w[\<option\>] can be used to enable different options based on the platform or configuration.  
...
- Release mode  
    Turns on -Ofast and other compiler specific optimizations, and removes any unnecessary information from the executable.  
