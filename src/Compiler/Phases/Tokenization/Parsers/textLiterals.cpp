 ...
 */
const char* getLiteralName(const cmp::DelimitedLiteralType literalType) {
    return literalType == cmp::DelimitedLiteralType::STRING ? "String" : "Char";
}
...
/**
 * @brief Parses the delimited (char or string) literal that starts at index <index> and checks if it contains a single 8-bit character
 ...
 */
cmp::TokenValue* cmp::parseDelimitedLiteral(pre::SegmentedCleanSource *b, ulong index, ulong *rawLiteralLen, DelimitedLiteralType literalType) {
    std::stringstream r;
    const char delimiter = literalType == DelimitedLiteralType::STRING ? '"' : '\'';
...
//FIXME merge with string logic. they are ideantical but use different names and delimiters
    ...
    if(literalType == DelimitedLiteralType::CHAR) {
    ...
    *rawLiteralLen = i - index;
    if(literalType == DelimitedLiteralType::STRING) {
 ...
 */
std::optional<std::string> cmp::decodeEscapeSequence(pre::SegmentedCleanSource *b, ulong index, ulong *rawEscapeLen, DelimitedLiteralType literalType) {
    std::stringstream r;
