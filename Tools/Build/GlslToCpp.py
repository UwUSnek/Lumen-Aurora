...
def parseShader(pathr:str, etop:str, isEngine:bool):
    ...
    code:str = subprocess.check_output(
        ['glslangValidator', f'{ shPath }/{ shName }.comp', '-E'],
    ...
    #Write output files
    with open(f'{ shPath }/{ shName }.gsi.hpp', 'w') as fh, open(f'{ shPath }/{ shName }.gsi.cpp', 'w') as fc:
        ...
        fc.write(fixTabs(
            ...
            f'\n// Changes could be overwritten without notice'
            f'\n//####################################################################################'
            f'\n#include "{ os.path.relpath(shPath, "." if isEngine else etop) }/{ shName }.gsi.hpp"'
            f'\n'
...
exit(parseShader(sys.argv[1], sys.argv[2], sys.argv[3] == 'e'))