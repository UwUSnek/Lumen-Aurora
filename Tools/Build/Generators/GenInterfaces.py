...
import re, subprocess, pathlib, Utils as u
...
#FIXME calculate array offset
    ...
    glsl):                                                      #
        ...
        offset = u.roundUp(offset, align)                             # Recalculate element offset
    ...
    # return dict({ 'func' : ret, 'ext' : ext, 'size' : u.roundUp(offset, max(maxAlign, 16)) + 64})    #BUG ok
    # return dict({ 'func' : ret, 'ext' : ext, 'size' : u.roundUp(offset, max(maxAlign, 16))})         #BUG crash
    return ns(**{
        'elms' : elms,
        'size' : u.roundUp(offset, max(maxAlign, 256)) #FIXME USE CALCULATED MAX ALIGNMENT
        #TODO RETURN STRUCT ALIGNMENT
    })
    # return dict({ 'cpp' : cpp, 'types' : types, 'size' : u.roundUp(offset, max(maxAlign, 256)) })
...
def parseShader(pathr:str, out:str, EtoA:str, isEngine:bool):
    ...
    code:str = u.clearGls(u.preprocessGls(os.path.relpath(pathr, ".")))
    ...
    with open(fhn, 'w') as fh, open(fcn, 'w') as fc:
        ...
        for l in layouts + externs:
            ...
            s += (
                ...
                # Copy constructor
                f'\n    inline { l.cstr }(const { l.cstr }& p{ u.capitalize1(l.name) }) {{'
                f'\n    	ShaderElm_b:: data = p{ c1name }. data;'
                f'\n    	ShaderElm_b::vdata = p{ c1name }.vdata;'
                f'\n    }}'

                # Copy assignment
                f'\n    inline { l.cstr }& operator=(const { l.cstr }& p{ u.capitalize1(l.name) }) {{'
                f'\n        ShaderElm_b:: data = p{ c1name }. data;'
                f'\n        ShaderElm_b::vdata = p{ c1name }.vdata;'
                f'\n        return *this;'
            ...
            s += f'\npublic:'
            for m in l.elms:
                getName = f'{ "e" if l.iExt else "l" }{ u.capitalize1(m.name) }'
        ...
        for e in externs:
            s += f'\n    const { e.cstr }& p{ u.capitalize1(e.name) },'
