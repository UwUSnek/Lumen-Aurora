...
static struct OperatorData_t* operator_get_data(const enum TokenID id){
	static struct OperatorData_t data[] = { // Sorted by length
		...
		{ ot_inc_dec,     2, 'l' }, { ot_inc_dec,     2, 'l' }, { ot_seq,        17, 'l' },
		...
		{ ot_ternary,    15, 'r' }, { ot_ternary,    15, 'r' }, { ot_field,       2, 'l' }, { ot_group,       1, 'n' }, { ot_group,       1, 'n' }, { ot_subscript,   2, 'l' }, { ot_subscript,   2, 'l' }, { ot_scope,       1, 'n' }, { ot_scope,       1, 'n' }
		//! Both ++ and -- can be prefix or postfix. The actual precedence and associativity is determined after the tokenization
		//! Same with +, -, ~. They can be unary or arithmetic
	};
	return data + id - o_start;
}


static const char* operator_to_string(const enum TokenID id){
