...
def run(dir):
	...
	with open('./.vscode/tasks.json', 'r') as f:
		s = re.sub(r'("args"\s*:\s*\[\s*"--mode=)[l|w][d|r|s]"', r'\g<1>' + opts.pf() + opts.tp() + '"',
			...
			re.sub(r'("label"\s*:\s*" > Switch to )(Linux|Windows)("(.|\n)*?".*?Lynx\/Tools\/Setup\/SetPlatform\.py"\s*,\s*")(l|w)"', r'\g<1>' + ("Windows" if opts.pf() == "l" else "Linux") + r'\g<3>' + ('w' if opts.pf() == 'l' else 'l') + '"',
			re.sub(r'("label"\s*:\s*" > Switch to )(Debug|Release)("(.|\n)*?".*?Lynx\/Tools\/Setup\/SetType' r'\.py"\s*,\s*")(d|r)"', r'\g<1>' + ("Release" if opts.tp() == "d" else "Debug") + r'\g<3>' + ('r' if opts.tp() == 'd' else 'd') + '"',
		f.read()))))
