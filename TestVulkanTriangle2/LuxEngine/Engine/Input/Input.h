...
namespace lux::input {
	...
	static void mouseAxisCallback(GLFWwindow* window, double x, double y) {
		*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).wd0 -= y * 10;
		*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).wd1 -= y * 10;
	}


	static void mouseCursorPosCallback(GLFWwindow* window, double x, double y) {
		////renderFence.wait(1);
		//*(*(rcast<Line2D*>(engine.objs[0]))).p1 = vec2i32{ (int32)x, (int32)y };
		////renderFence.set(0);
	}
	...
	static void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
		if (key == LUX_KEY_KP_SUBTRACT) (*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col0).w = max((*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col0).w - 0.05, 0);
		if (key == LUX_KEY_KP_SUBTRACT) (*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col1).w = max((*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col1).w - 0.05, 0);
		if (key == LUX_KEY_KP_ADD) if ((*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col0).w < 1) *(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col0 += vec4f32(0, 0, 0, 0.05);
		if (key == LUX_KEY_KP_ADD) if ((*(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col1).w < 1) *(*(rcast<lux::obj::Line2D*>(engine.objs[0]))).col1 += vec4f32(0, 0, 0, 0.05);
		...
		static uint16 yMin = 0, yMax = scast<uint16>(lux::input::inStates->sequences.size() - 1), x = 0;
		...
		if (action != GLFW_REPEAT) {										//If the action is not repeat
			...
			while (true) {														//Now find the maximum y 
				if (keyCode == lux::input::inStates->sequences[i].sequence[x]) {		//if the input key is NOT equal to the key of the input state sequence
					...
					if (i >= lux::input::inStates->sequences.size()) {					//If there are no more different keys
						yMax = scast<uint16>(lux::input::inStates->sequences.size() - 1);	//Set the maximum y as the maximum index of the sequences
						break;																//Exit the loop
					}
				}
			...
			// UwU
			if (yMin == yMax && x == scast<LuxArray<uint16>>(lux::input::inStates->sequences[yMax].sequence).size() - 1) {	//If the maximum and minimum y are the same
				...
				yMax = scast<uint16>(lux::input::inStates->sequences.size() - 1);												//Reset the maximum y
				x = 0;																											//Reset the x
				return;																											//Exit the function
			}
