...
public:
	...
	//Adds a LuxDynArray to the array, skipping all the invalid elements and placing the others in the first free index
	...
	LuxArray<uint64> add(LuxDynArray<type> vec) {
		LuxArray<uint64> IDs;
		IDs.resize(vec.size());								//Set the number of IDs
