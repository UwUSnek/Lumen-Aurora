...
namespace lux {
	template<class type, class alloc = uint32> struct DynArray : public Array<type, alloc> {
		alloc chunkSize;
		...
		inline DynArray(const alloc vChunkSize = fit(sizeof(type), 500000)) : chunkSize{ vChunkSize } { Array<type, alloc>::Array(); }
		...
		inline alloc __vectorcall resize(const alloc vNewSize) {
			Array<type, alloc>::resize((vNewSize == 0) ? 0 : step(vNewSize, chunkSize));
		...
		inline alloc __vectorcall resize(const alloc vNewSize, const type& vInitValue) {
			Array<type, alloc>::resize((vNewSize == 0) ? 0 : step(vNewSize, chunkSize), vInitValue);
			return this->__lp_size = vNewSize;
		}
