...
template <class cntType, class alloc> class LuxContainer;
...
public:
	alloc __lp_dynSize;			//Allocated number of elements. Please use size() instead of this variable
	alloc __lp_freeNum;			//Number of free elements in the map. Please use usedSize() or freeSize() instead of this variable
	type** __lp_data;			//Map elements
	alloc** __lp_tracker;		//Map that stores the state of each element

private:
	alloc head;					//First free element
	alloc tail;					//Last free element
	alloc chunkSize;			//Size of the chunks
	alloc maxSize;				//Maximum number of elements in the map
...
public:
	...
	LuxMap(const alloc vChunkSize = fit(sizeof(type), 500000), const alloc vMaxSize = fit(sizeof(type), 500000) * 0xFFFF) : 
	...
	LuxMap(const LuxContainer<elmType, alloc>* in) {
	...
	alloc __vectorcall append(const type vData) {
		if (__lp_dynSize + 1 > chunksDynNum * chunkSize) {									//If the chunk is full
			...
			__lp_tracker[chunksDynNum] = scast<alloc*>(malloc(sizeof(alloc*) * chunkSize));		//Allocate a new tracker chunk
	...
	//Returns the ID of the element
	alloc __vectorcall add(const type vData) {
		alloc head2 = head;
		if (head == (alloc)-1) return append(vData);		//If it has no free elements, append it
		else {
	...
	LuxArray<alloc> __vectorcall add(const std::vector<type>* pVector) {
		LuxArray<alloc> IDs;
	...
	LuxArray<alloc> __vectorcall add(const LuxMap<type, alloc>* vContainer) {
		LuxArray<alloc> IDs;
	...
	signed char __vectorcall remove(const alloc vIndex, const bool vFreeElm = false) {
		...
		if (r != 0) return r;								//If the index is not valid, return the error code
			...
			if (head == scast<alloc>(-1)) head = tail = vIndex;//If it has no free elements, initialize head and tail.
	...
	inline void __vectorcall clear() {
		...
		__lp_data = (type**)malloc(sizeof(type*) * (maxSize / chunkSize));					//Allocate data map
		__lp_tracker = scast<alloc**>(malloc(sizeof(alloc*) * (maxSize / chunkSize)));		//Allocate tracker map
	...
	inline signed char __vectorcall status(const alloc vIndex) const {
		if (vIndex == (int64)-1)return -1;								//Invalid index
		else if (vIndex >= __lp_dynSize) return -2;						//Index out of range
		else if (__lp_Tracker(vIndex) == scast<alloc>(-1)) return 0;	//Ok
	...
	inline type* __vectorcall data(const alloc vChunkIndex) const { return &__lp_data[vChunkIndex][0]; }
	...
	inline alloc const size() const { return __lp_dynSize; }
	//Returns the number of used elements
	inline alloc __vectorcall usedSize() const { return __lp_dynSize - __lp_freeNum; }
	//Returns the number of free elements
	inline alloc freeSize() const { return __lp_freeNum; }
};
