...
void Render::createTextureSampler() {
	...
	if (vkCreateSampler(graphics.LD, &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS) Quit("Failed to create texture sampler");
}
...
void Render::createImage(uint32 width, uint32 height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
	...
	if (vkCreateImage(graphics.LD, &imageInfo, nullptr, &image) != VK_SUCCESS) Quit("Failed to create image");
	...
	if (vkAllocateMemory(graphics.LD, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) Quit("Failed to allocate image memory");
...
void Render::cleanupSwapChain() {
	...
	for (int64 i = 0; i < swapChainImages.size(); i++) {
		vkDestroyBuffer(graphics.LD, uniformBuffers[i], nullptr);
		vkFreeMemory(graphics.LD, uniformBuffersMemory[i], nullptr);
	}
...
//Add validation layers if in debug mode
	...
	//Create instance
	if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS) Quit("Failed to create instance");
}
...
void Render::createRenderPass() {
	...
	//Create render pass
	if (vkCreateRenderPass(graphics.LD, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) Quit("Failed to create render pass");
}
...
void Render::createDescriptorSetLayout() {
	...
	uboLayoutBinding.binding = 0;										//Binding point in shader
	uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;//Type of descriptors
	uboLayoutBinding.descriptorCount = 1;								//Number of descriptoes
	...
	if (vkCreateDescriptorSetLayout(graphics.LD, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
...
void Render::createFramebuffers() {
	...
	for (int64 i = 0; i < swapChainImageViews.size(); i++) {
		...
		if (vkCreateFramebuffer(graphics.LD, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) Quit("Failed to create framebuffer");
	}
}
...
void Render::createDescriptorPool() {
	...
	if (vkCreateDescriptorPool(graphics.LD, &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS) Quit("Failed to create descriptor pool");
}
...
void Render::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
	...
	if (vkCreateBuffer(graphics.LD, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) Quit("Failed to create buffer");
	...
	if (vkAllocateMemory(graphics.LD, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) Quit("Failed to allocate buffer memory");
...
void Render::drawFrame() {
	...
	if (vkQueueSubmit(graphics.graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) Quit("Failed to submit draw command buffer");
	...
	else if (result != VK_SUCCESS) Quit("Failed to present swapchain image");
