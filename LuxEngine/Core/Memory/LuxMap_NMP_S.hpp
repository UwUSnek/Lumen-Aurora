...
namespace lux {
	...
	public:
		...
		//Array<iter> add(const std::vector<type>* pVector) {
		//	Array<iter> IDs;
		...
		bool remove(const iter vIndex, const bool vFreeElm = false) {
		...
		inline type& operator[](const iter vIndex) const { return __lp_Data(vIndex); }
		//Returns a pointer to the first element of a chunk. The elements are guaranteed to be in contiguous order
		inline type* begin(const iter vChunkIndex) const { return &data_[vChunkIndex][0]; }
		...
		inline iter size( ) const { return size_; }
		//Returns the number of used elements
		inline iter usedSize( ) const { return size_ - freeSize_; }
