...
namespace lux {
	...
	public:
		...
		//inline uint64 __vectorcall resize(const iter vNewSize) {
		...
		inline uint64	__vectorcall count( )	const override { checkInit; return count_;					}
		inline uint64	__vectorcall size( )	const override { checkInit; return count_ * sizeof(type);	}
		inline bool		__vectorcall empty( )	const override { checkInit; return !count_;					}
		inline type*	__vectorcall begin( )	const override { checkInit; return data_;					}
		inline type*	__vectorcall end( )		const override { checkInit; return &data_[count_];		}

		inline type&	__vectorcall operator[](const uint64 vIndex) const {
...
//
//
//		inline iter		__vectorcall count( )	const override { checkInit; return size_;					}
//		inline uint64	__vectorcall size( )	const override { checkInit; return size_ * sizeof(type);	}
