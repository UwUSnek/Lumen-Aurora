...
namespace lux::core::c{
	...
	void cshaderCreateDefaultCommandBuffers( ) {
		{ //Copy
			...
			commandBufferAllocateInfo.commandBufferCount = lux::getEngine( ).swapchainImages.size( );
			...
			for(uint32 imgIndex = 0; imgIndex < lux::getEngine( ).swapchainImages.size( ); imgIndex++) {
				...
				readToWrite.image = lux::getEngine( ).swapchainImages[imgIndex];			//Set swapchain image
				...
				writeToRead.image = lux::getEngine( ).swapchainImages[imgIndex];		//Set swapchain image
				VkPipelineStageFlags 								//Create stage flags
