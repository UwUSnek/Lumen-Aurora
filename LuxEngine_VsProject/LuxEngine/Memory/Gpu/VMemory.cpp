	...
	Cell alloc(const uint64 vSize, CellClass vCellClass, const AllocType vAllocType){
		...
		if(vCellClass == LUX_CELL_CLASS_0){							//If the cell is a custom size cell
			Cell cell{																	//Create the cell object
				...
				.buffer = &typeBuffers[typeBuffers.add(MemBuffer{ 0, 0, Map<bool, uint32>(1, 1) })],
				.cellIndex = 0,
			};
			...
			core::c::buffers::createBuffer(
				...
				&cell.buffer->buffer, &cell.buffer->memory
			);
			...
			{ //If there are none, create a new buffer and allocate the cell in it
				...
				cell.buffer = &typeBuffers[typeBuffers.add(MemBuffer{ 0, 0, Map<bool, uint32>(bufferSize / vCellClass, bufferSize / vCellClass) })];
				cell.cellIndex = cell.buffer->cells.add(true);
				core::c::buffers::createBuffer(
