...
namespace lux::rem{
	...
	Cell alloc(const uint64 vSize, CellClass vCellClass, const AllocType vAllocType){
		...
		if((uint32)vCellClass){																//If the cell is a fixed size cell
			...
			MemBuffer& buffer = subBuffers[subBuffers.add(MemBuffer{ 0, 0, !(uint32)vCellClass ? Map<Cell_t, uint32>(1, 1) : Map<Cell_t, uint32>(bufferSize / (uint32)vCellClass, bufferSize / (uint32)vCellClass) })];
			...
			cell->cellIndex = !(uint32)vCellClass ? 0 : cellIndex;								//Add a new cell and set the cell index
